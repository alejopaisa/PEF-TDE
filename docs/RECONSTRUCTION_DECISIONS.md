...
|---|---|
| Decision ID | DEC-007 |
| Title / Topic | Scientific principles for reconstructed temporal fitting-window selection |
| Scientific Question | What criteria must govern the future reconstructed temporal-window choice before any numerical window is selected? |
| Historical Method | The exact historical fitting-window rule is not sufficiently recoverable to adopt as the reconstructed rule. |
| Historical Evidence Level | D for the exact historical fitting-window decision logic. |
| Reconstruction Decision | The final reconstructed temporal fitting window must be selected according to model-independent, reproducible, physically and observationally motivated principles before examining model-fit preference, information criteria, reconstructed classifications, or residual performance. Temporal-window selection must not use AIC, AICc, BIC, chi-square improvement, residual quality, recovered tau, fitted floor strength, reconstructed model classification, or agreement with historical classification as criteria for choosing the window. Source MJD remains the canonical timestamp under DEC-003. Any temporal-window interval used for reconstructed model selection must be expressed consistently in rest-frame elapsed days according to DEC-005, `delta_t_rest = delta_t_observer / (1 + z_adopted)`, using DEC-004 adopted redshifts. Absolute MJD itself is not converted into a rest-frame MJD. The final fitting window must be explicitly bounded rather than silently using the entire available multi-year eligible baseline. ALL_ELIGIBLE and POST_REFERENCE_ALL remain sensitivity/comparison scenarios and are not automatically adopted as the final fitting window. This principle does not yet choose the numerical lower or upper bound. The beginning of the fitting interval must correspond to an explicitly defined observational phase appropriate to modeling the TDE decline. The currently recorded publication/reference epochs are heterogeneous descriptive anchors and are not sufficient by themselves to establish a common physical decline start across all events. DEC-007 does NOT adopt the current publication/reference epoch as the final fitting-window start, model zero, t0, t_ref, or C_event. A separate audit/decision is required to define the decline-phase anchor used for final fitting-window construction. For every future candidate/final rule, the project must report retained measurement count per event-band, empty event-bands, sparsely sampled event-bands, retained temporal span, largest internal temporal gaps, and pre/post-anchor coverage where applicable. No arbitrary minimum-N threshold is frozen by DEC-007. In particular, a window must not be extended solely to force a desired measurement count in a sparsely sampled instrument/band. If a proposed window yields zero measurements for an event-band, that outcome must be explicitly reported and resolved before model fitting. DEC-007 does not yet decide whether such a band would cause rejection of the candidate window, exclusion of the band, or another documented treatment. Measurements separated by large observational gaps must not be described or interpreted as continuous temporal coverage. Any final fitting-sample construction must preserve/report internal gaps and must not infer continuous event evolution merely from the total span between first and last retained observations. DEC-007 does not yet impose a numerical maximum-gap threshold. The temporal selection rule should be scientifically defined independent of instrument identity wherever possible. WISE W1/W2 measurements must not be included or excluded merely to satisfy an arbitrary sampling-count threshold. Any instrument-specific exception would require separate explicit scientific justification. The final temporal-window rule must be deterministic, machine-readable, reproducible from source MJD and frozen metadata, and documented independently of model-fitting outcomes. |
| Scientific Justification | TEMP-001 through TEMP-004 and the candidate temporal-window audit show strongly heterogeneous temporal coverage, including multi-year spans, large gaps, different sampling densities, and materially different retained sample sizes across candidate windows. The candidate-window audit demonstrates that numerical window choice materially changes sample size and sampling structure: +180-day scenarios can leave an event-band empty; +365-day scenarios leave several very sparsely sampled bands; those fewer-than-five-point cases are specifically the eight W1/W2 datasets; and +730-day scenarios increase W1/W2 sampling, but that fact alone is not sufficient scientific justification for selecting +730 days. The eight W1/W2 cases demonstrate why a simple retained-count threshold could make the window choice depend primarily on instrument cadence rather than event physics. Heterogeneous reference anchors prevent treating equal numerical offsets as automatically equivalent physical phases across events. These findings constrain the decision process but do not rank candidate windows. |
| Affected Data | No raw, processed, reconstructed eligible, or candidate-window audit data are modified by this decision. Future fitting-sample construction must apply a separately frozen temporal-window rule derived from these principles. |
| Affected Results | Future reconstructed fitting samples, model fits, classifications, diagnostics, tables, figures, and paper text that depend on the final temporal fitting window. |
| Affected Paper Sections | Fitting Procedure; Results; Robustness; Discussion; Appendices; any reconstructed tables or figures describing the final fitting sample. |
| Status | FROZEN |
| Date Frozen | 2026-09-24 |

DEC-007 does not choose the final lower temporal bound, final upper temporal bound, any REF_* scenario, POST_REFERENCE_ALL, ALL_ELIGIBLE, exact minimum retained N, exact maximum allowed gap, final decline-phase anchor, WISE inclusion/exclusion, pre-reference inclusion/exclusion, C_event, t0, t_ref, parameter bounds, optimizer, or model-selection criterion.

The final fitting window remains unresolved.
